...
if __name__ == '__main__':
    ...
    results_path = "C:\Users\edz\OneDrive @ Tobii Technology AB\PhD\Chapter 2 - Gaze Contingency\Analysis\Results"
    if not path.exists(results_path):
        makedirs(results_path)

    #Open a file to write the results to a txt file
    f = open(sep + 'Results' + sep + 'number_of_trials_per_percentage.txt', 'w')
    ...
    number_trials_valid_data = np.zeros((len(part_name), 6), dtype=float)
    ...
    for j, participant in enumerate(folders):
        ...
        f_perc_data = open(results_path + sep + participant + sep + 'percentage_of_valid_data.txt', 'w')
        f_onset = open(results_path + sep + participant + sep + 'et_data_indexes_every_stimulus_onset.txt', 'w')

        #import pdb; pdb.set_trace()
        #Open files
        #Onset of every stimulus
        onset_data = np.loadtxt(part_path + sep + onset_file_name[0], dtype=([('Part', np.int), ('Cond', np.int),
        ...
        for onset in onset_data:
            indexes.append(starting_index + find_timestamp(gaze_data['Local_timestamp'][starting_index:], onset['Time']*1000000))
        ...
        write_file(f_onset, indexes_str)
        f_onset.close()
...
'''
ERROR WHEN SAVING THE TXT FILE, INDEXES --> CONVERT IT CORRECTLY TO A STRING!
'''
